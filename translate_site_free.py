...
def restore_brands(text):
    for brand, placeholder in brand_placeholders.items():
        ...
        # Just in case translation changes casing of placeholder slightly
...
def translate_text(text, target_lang):
    ...
    # Check if text is just email or URL or phone number
    ...
    for attempt in range(5):
        try:
            ...
            print(f"Error translating '{text[:30]}' (Attempt {attempt+1}): {e}")
            sys.stdout.flush()
            time.sleep(2.0)
...
def translate_html_file(filename, target_langs):
    ...
    for lang in target_langs:
        ...
        for element in soup.descendants:
            if isinstance(element, NavigableString) and not isinstance(element, Comment):
                ...
                if parent and parent.name not in ['script', 'style', 'head', 'title', 'meta', '[document]']:
                    ...
                    # Only translate if there's actual alphabetic text and length > 1
                    if len(text) > 1 and any(c.isalpha() for c in text):
                        # Skip language switcher modal and nav-lang links to prevent double translation or link break
                        is_lang_el = False
                        p = parent
                        while p:
                        ...
                        if not is_lang_el:
                            ...
                            texts_to_translate.append(str(element)) # Keep exact spacing
                            
        print(f"Found {len(nodes_to_translate)} text nodes to translate.")
        sys.stdout.flush()
        
        # 2. Translate and replace node-by-node with progress prints
        for idx, node in enumerate(nodes_to_translate):
            orig_text = texts_to_translate[idx]
            
            # Print progress every 15 nodes
            if idx % 15 == 0 or idx == len(nodes_to_translate) - 1:
                print(f"  Progress: {idx+1}/{len(nodes_to_translate)} nodes...")
                sys.stdout.flush()
                
            translated = translate_text(orig_text, lang)
            if translated:
                node.replace_with(translated)
            
            time.sleep(0.04) # 40ms polite delay to prevent Google Translate rate limits
            
        # 3. Update local href links (prevent link breaking)
        ...
        # 4. Save file
        print(f"Saving translated file: {out_name}")
        sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    print(f"Starting Free Google Translate Engine for {len(files_to_translate)} files across {target_languages}...")
    sys.stdout.flush()
    
    for filename in files_to_translate:
        translate_html_file(filename, target_languages)
        
    print("\n🎉 Free Google Translate site translation completed successfully!")
    sys.stdout.flush()
