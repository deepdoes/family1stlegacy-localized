...
"""
    ...
    # Try with up to 5 retries with backoff for rate limits
    backoff = 4.0
...
def translate_html_gemini(filepath, target_langs, api_key, overwrite=True):
    ...
    for lang in target_langs:
        ...
        print(f"\n--- Translating to {lang} via block-level Gemini 2.5 Flash ---")
        ...
        if html_fragments:
            # 2. Batch translate (Premium API permits large chunk sizes safely!)
            chunk_size = 20 
...
if __name__ == "__main__":
    ...
    print(f"Starting Tag-Preserving Block-level Gemini 2.5 Flash Engine across {len(files_to_translate)} pages...")
    for filename in files_to_translate:
        filepath = os.path.join(base_dir, filename)
        if not os.path.exists(filepath):
            print(f"Warning: File not found {filepath}")
        else:
            translate_html_gemini(filepath, ['es', 'pt', 'sw', 'rw'], api_key, overwrite=True)
            
    print("\n🎉 Tag-Preserving Block-level Gemini Premium Translation Pipeline Completed Successfully!")
