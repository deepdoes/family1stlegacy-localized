...
"""
    ...
    backoff = 75.0 # Sleep for 75 seconds immediately on 429 to allow rolling token release!
    for attempt in range(10): # Let's do up to 10 attempts!
        try:
            ...
            sys.stdout.flush()
            if "429" in str(e):
                print(f"Rate limited (429). Sleeping for {backoff} seconds to allow rolling quota to release tokens...")
                sys.stdout.flush()
                time.sleep(backoff)
                # Keep sleep constant or slightly increase to guarantee success
                backoff = min(120.0, backoff + 15.0)
            else:
                time.sleep(3)
...
def translate_html_groq(filepath, target_langs, api_key):
    ...
    for lang in target_langs:
        ...
        print(f"\n--- Translating to {lang} via block-level Llama 3.1 8B Engine ---")
        sys.stdout.flush()
        ...
        for i in range(0, len(html_fragments), chunk_size):
            ...
            if res:
                ...
                print("Batch translation failed. Attempting item-by-item fallback...")
                sys.stdout.flush()
                time.sleep(3.0)
                for item in chunk:
                    ...
                    if item_res and len(item_res) == 1:
                        ...
                        print(f"Fallback item failed, keeping original...")
...
if __name__ == "__main__":
    ...
    print("Translating terms.html to Kinyarwanda (rw) via Groq Llama 3.1 8B with self-healing pacing...")
    sys.stdout.flush()
    translate_html_groq(filepath, ['rw'], api_key)
